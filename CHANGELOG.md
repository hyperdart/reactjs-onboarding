...
### Changed
- Removed `prop-types` peer dependency — the library no longer requires or validates PropTypes, reducing install footprint

### Fixed
- Elements outside the viewport are now automatically scrolled into view before the spotlight renders, so off-screen steps no longer appear at the wrong position
